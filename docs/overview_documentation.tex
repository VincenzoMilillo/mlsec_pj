...
\subsection{APoZ Diagnostic and Post-ReLU Mapping}

The earlier diagnostic run observed 10,250 units but only 14 unique APoZ scores, with 98.34\% of units receiving a zero score. This showed that counting near-zero values directly on raw \code{Conv2d} and \code{Linear} outputs produced a weak ranking.

The current implementation instead attaches hooks to DenseNet \code{ReLU} modules and measures the percentage of actual zero activations for each channel. The initial \code{relu0} channel is mapped to the corresponding output channel of \code{conv0}. Internal \code{relu1}, \code{relu2}, and transition \code{relu} channels are mapped to the matching input channel of the convolution that consumes that activation. Channels are ordered from highest to lowest APoZ before their connected flattened weight indexes are passed to the injector. Any weights not covered by these mappings are appended using the least-absolute-value order. The revised diagnostic also reports the number of mapped ReLU modules and the number of weights covered directly by APoZ. A new controlled run is required to compare this corrected ranking with the earlier implementation and the baseline.
