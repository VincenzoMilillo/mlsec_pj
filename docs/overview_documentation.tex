...
\hline
\endhead
README.md & Main project description, warnings, dependency notes, and basic command-line usage. \\
RUN\_SAMPLE.md & Notes from a sample run, including a dummy payload workflow and verification instructions. \\
SETUP\_MAC\_PROM4\_REPORT.md & Environment and compatibility report for running the project on macOS Apple Silicon with PyTorch MPS. \\
requirements.txt & Pinned Python dependencies used by the current setup. \\
maleficnet.py & Main command-line entry point. It wires together the dataset, model, injector, extractor, trainer, device selection, checkpoints, and run modes. \\
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \code{checkpoints/}: pre-injection and post-injection model weights saved by \code{maleficnet.py}.
  \item \code{payload/}: location where input payloads are expected.
...
\subsection{\code{maleficnet.py}}

\code{maleficnet.py} is the orchestrator. It performs device detection, parses command-line arguments, initializes the data module and model, prepares the injector and extractor, trains or loads a model, injects the payload, retrains after injection, and finally checks whether extraction succeeds.
...
--model, -m & vgg11 & Model name argument. The implemented model initializer recognizes \code{densenet}; practical runs should pass \code{--model densenet}. \\
...
--epochs & 60 & Number of epochs for training or fine-tuning. \\
--batch\_size & 64 & DataLoader batch size. \\
--random\_seed & 8 & Seed passed to \code{torch.manual\_seed}. \\
--num\_workers & 20 & Number of worker processes for data loading. \\
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Initialize \code{Injector} with the payload path, output path, seed, device, and chunk factor.
  \item Initialize \code{Extractor} with the same seed, output path, payload length, hash length, and chunk factor.
...
Selected positions are generated with a deterministic pseudo-random index sequence:
...
The same seed and selection logic are used during extraction, so the extractor knows which weights to read without storing the index list separately.
...
\code{maleficnet.py} configures Python logging to write to \code{maleficnet.log}. The injector and extractor log timing, signal-to-noise estimates, hash comparison values, and success/failure status.

\subsection{Training Logging}

The main script attempts to use PyTorch Lightning's \code{TensorBoardLogger}. If that import fails, it sets the trainer logger to \code{None}. The repository also contains \code{logger/csv\_logger.py}, a minimal logger that can write training and validation metrics to CSV files. This logger implements methods and properties expected by Lightning, including \code{log\_metrics}, \code{log\_hyperparams}, \code{name}, \code{version}, \code{experiment}, \code{finalize}, \code{log\_graph}, \code{log\_dir}, and \code{save}.
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item \code{maleficnet.py} initializes the data module and DenseNet model.
  ...
  \item \code{Injector} embeds the resulting signal into flattened model weights.
  \item The injected model is retrained and saved.
  \item \code{Extractor} reads the same weight positions using the same seed.
...
The argument parser defaults \code{--model} to \code{vgg11}, but \code{initialize\_model} only creates a model when \code{model\_name == "densenet"}. Therefore, practical commands should explicitly pass \code{--model densenet}.
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item Implementing a model-weight analyzer to choose lower-risk weights rather than selecting positions uniformly at random.
...
The project is a compact end-to-end demonstration of hiding a binary payload in neural-network parameters using spread-spectrum techniques and LDPC channel coding. The source tree separates the workflow cleanly: \code{maleficnet.py} orchestrates the run, \code{models/} and \code{dataset/} define the learning task, \code{injector.py} implements embedding, \code{extractor.py} implements recovery, and \code{utils/} provides low-level bit operations. The main technical tradeoff is between maintaining model performance and keeping the embedded signal recoverable after training noise.
