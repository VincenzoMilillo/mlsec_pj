# Difference from extractor.py:
# this variant does not choose extraction indexes internally with random
# sampling. Instead, extract() receives an external extract_sequence, so the
# same extractor can be used with smarter analyzer strategies that select the
# model weights to read from. It also builds the preamble with np.random.choice
# over {-1, 1}, matching the sequence format expected by the new workflow.
...
class Extractor:
    ...
    def extract(self, model, message_length, malware_name, extract_sequence):
        ...
        with tqdm(total=message_length) as bar:
            ...
            for _ in range(message_length):
                ...
                current_extract_sequence_index = extract_sequence[current_chunk * self.CHUNK_SIZE * self.chunk_factor:
                                                      (current_chunk + 1) * self.CHUNK_SIZE * self.chunk_factor]
                current_models_w_delta = models_w_curr[current_extract_sequence_index]
        ...
        preamble = np.random.choice([-1,1], size = 200)
