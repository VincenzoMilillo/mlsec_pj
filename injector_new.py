# Difference from injector.py:
# this variant does not choose injection indexes internally with random
# sampling. Instead, inject() receives an external inject_sequence, so the same
# injector can use analyzer strategies that rank or select the model weights to
# modify. The extractor must receive the matching sequence during recovery.
...
class Injector:
    ...
    def inject(self, model, inject_sequence, gamma: Optional[float] = None):
        start = time.time()
        ...
        with tqdm(total=len(b)) as bar:
            ...
            for bit in b:
                ...
                current_inject_sequence_index = inject_sequence[current_chunk * self.CHUNK_SIZE * self.chunk_factor:
                                                      (current_chunk + 1) * self.CHUNK_SIZE * self.chunk_factor]
                
                models_w[current_inject_sequence_index] = np.add(
                    models_w[current_inject_sequence_index], current_bit_cdma_signal)
